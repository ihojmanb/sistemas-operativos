...
struct Task { // Descriptor de una tarea
  ...
  /* para nCombineOxy, nCombineHydro */
  void *oxy;
  void *hydro;
  int molecula_ready;
  void *molecula;
...
#define WAIT_COMBINE 13 // hizo nCombineOxy o nCombineHydro y espera a que
// lleguen más atomos
...
#define STATUS_LIST {"READY", "ZOMBIE", "WAIT_TASK", "WAIT_REPLY", \
                     ...
                     "WAIT_COND_TIMEOUT", "WAIT_SLEEP", "WAIT_COMBINE" }
...
void START_CRITICAL(void); // deshabilitacion de interrupciones
void END_CRITICAL(void);   // habilitacion de interrupciones
